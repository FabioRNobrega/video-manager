...
**New playlist route and layout, single shared player instance.** Add `WebApp/WebApp.Client/Pages/PlaylistView.razor` at `@page "/playlist/{Category}/{FolderId}"`, following the existing routed-page pattern (`Videos.razor`, `Music.razor`, etc.) and the client-owned routing boundary in `AGENTS.md`. On `OnParametersSetAsync`, it calls the new `GET /api/archive/{category}/items/{id}/playlist` endpoint and calls `PlayerState.EnterPlaylistView(category, folderId, folderName, listing.Items)`; on `Dispose`, it calls `PlayerState.ExitPlaylistView()`. The page itself renders only loading/empty/error states — the actual player+queue layout is a self-contained `WebApp/WebApp.Client/Components/PlaylistLayout.razor` rendered directly by `PlaylistView.razor`. `MainLayout.razor` needs only one small change to guarantee a single mounted player: it renders its persistent footer `<Player PlayerState="PlayerState" Mode="PlayerPresentationMode.Footer" />` only when `PlayerState.Selected is not null && !PlayerState.PlaylistViewActive`, so while the playlist route's own `<Player Mode="PlayerPresentationMode.Playlist" />` (inside `PlaylistLayout.razor`) is mounted, the footer instance is not. Leaving the playlist route disposes `PlaylistView`/`PlaylistLayout`/its embedded `Player`, `ExitPlaylistView()` clears `PlaylistViewActive`, and `MainLayout.razor` resumes rendering the footer-mode `Player`.

Because each remount creates a fresh `Player.razor`/`MediaPlayerState`/media element, playback position and playing state do not survive across that boundary unless something outside the component preserves them. `PersistentPlayerState` therefore also tracks `LastKnownTime`/`WasPlaying`, updated via a new `UpdatePlaybackProgress(currentTime, isPlaying)` call from `Player.razor`'s existing `SynchronizeMediaStateAsync()` (already invoked on `timeupdate`/`play`/`pause`/etc., so no new event wiring is needed) — deliberately without raising `StateChanged`, since this is read on-demand at mount rather than something other UI needs to react to live. On `loadedmetadata` (the first point a freshly mounted media element can reliably seek), `Player.razor` seeks to `PlayerState.LastKnownTime` when it is greater than zero and resumes playback only if `PlayerState.WasPlaying`; the existing "autoplay a freshly selected track" behavior is preserved by gating it on `LastKnownTime <= 0` (a genuinely new selection always starts at zero, so autoplay-on-select is unaffected). Every method that represents a genuinely new selection (`Select`, `SelectPlaylistTrack` — covering `SelectMusic`, `SelectPlaylistItem`, `SelectNextTrack`, `SelectPreviousTrack`, and the first-time branch of `EnterPlaylistView`) resets `LastKnownTime`/`WasPlaying` to `0`/`false`, so a deliberately different track still starts fresh. `EnterPlaylistView` itself is also changed to check whether the currently selected item is still present in the freshly-loaded queue; if so, it only refreshes `Playlist`/`PlaylistFolderName`/`PlaylistCategory`/`PlaylistFolderId` and leaves `Selected`/`LastKnownTime`/`WasPlaying` untouched, instead of unconditionally jumping back to the first queue item — this is what makes re-opening the same playlist resume on the track the user was already on.

**Return-to-playlist control.** `PersistentPlayerState` also exposes `PlaylistCategory`/`PlaylistFolderId` (set by `EnterPlaylistView`, cleared whenever a non-playlist selection or an ad-hoc `SelectMusic` queue replaces it) and a derived `CanReturnToPlaylist`. `MediaPlayerControls.razor` gains a `ShowPlaylistButton`/`OpenPlaylist` parameter pair rendering a `bi-music-note-list` icon button in the same `btn-group` as the Fill-tab control; `Player.razor` computes `ShowPlaylistButton` as `PlayerState.CanReturnToPlaylist && !PlayerState.PlaylistViewActive` and navigates to `/playlist/{category}/{folderId}` on click via `NavigationManager`.
...
- `WebApp/WebApp.Client/Services/PersistentPlayerState.cs` - generalize `MusicPlaylist` into a mixed-kind `Playlist`, add `HasPlaylist`, `PlaylistViewActive`, `PlaylistFolderName`, `PlaylistCategory`, `PlaylistFolderId`, `CanReturnToPlaylist`, `LastKnownTime`, `WasPlaying`, `UpdatePlaybackProgress`, `EnterPlaylistView`, `ExitPlaylistView`, `SelectPlaylistItem`; keep `SelectMusic` as a compatible wrapper; make `EnterPlaylistView` resume an existing in-queue selection instead of always selecting the first item.
- `WebApp/WebApp.Client/Components/Player.razor` - add `PlayerPresentationMode` parameter driving layout CSS; generalize `HandleEndedAsync` auto-advance from `IsMusic` to `HasPlaylist`; restructure the playlist-mode viewport/controls to a bounded-height flex chain instead of a fixed-aspect-ratio box; call `PlayerState.UpdatePlaybackProgress` from `SynchronizeMediaStateAsync`; seek/resume from `PlayerState.LastKnownTime`/`WasPlaying` in `HandleLoadedMetadataAsync`; add `NavigationManager`-driven `OpenPlaylist`/`ShowPlaylistButton`.
...
- `WebApp/WebApp.Client/Components/MediaPlayerControls.razor` - add a `HasPlaylist` parameter for the previous/next gate, alongside the existing `IsMusicMode` gate for per-item control visibility; add a `ShowPlaylistButton`/`OpenPlaylist` parameter pair rendering a "Show playlist" icon button beside the Fill-tab control.
...
- `WebApp.Tests/Client/PersistentPlayerStateTests.cs` - cover `EnterPlaylistView`/`ExitPlaylistView`/`SelectPlaylistItem`, mixed-kind previous/next, unchanged music-only behavior, `UpdatePlaybackProgress` (including the no-op-when-nothing-selected and no-`StateChanged` cases), progress reset on every genuinely new selection, and `EnterPlaylistView` resuming vs. resetting selection/progress depending on whether the current selection is still in the (re-loaded) queue.
...
| Playback position/playing-state is lost across the playlist-route <-> footer-mode remount transition, or when re-entering the same playlist | The footer and playlist-mode players are the same component but a new instance (and a new `MediaPlayerState`) is created whenever `Player.razor` remounts, per Blazor component lifecycle; `EnterPlaylistView` originally always selected the first queue item regardless of an existing selection. | `PersistentPlayerState` now tracks `LastKnownTime`/`WasPlaying`, updated on every `timeupdate`/`play`/`pause` sync while a `Player.razor` instance is mounted; a newly-mounted instance seeks to `LastKnownTime` and resumes play only if `WasPlaying` once the media's `loadedmetadata` fires. `EnterPlaylistView` now checks whether the current selection is still in the (re-loaded) queue and, if so, preserves it instead of resetting to the first item. Selecting a genuinely new item (next/previous/queue click/different playlist) still resets `LastKnownTime`/`WasPlaying` to `0`/`false`, so the "new item starts at 0" behavior is unaffected. |
