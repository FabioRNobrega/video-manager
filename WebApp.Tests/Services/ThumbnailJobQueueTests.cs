...
public sealed class ThumbnailJobQueueTests
{
    [Fact]
    ...
    private static ThumbnailJobQueue CreateQueue(int capacity) =>
        new(Options.Create(new ThumbnailCacheOptions { Path = Path.GetTempPath(), QueueCapacity = capacity }));

    private static ThumbnailJob CreateJob(string cacheKey) =>
